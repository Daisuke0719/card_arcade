...
このゲームをレビューするのは **担当9（ページワン担当）** です。`npm run dev` でババ抜きを開き、次の3つを実際に操作してください。
ババ抜きの仕様を知らなくても、書いてあるとおりに動かせば判定できます。

**このコードを他人の目で見るのは、あなた1人だけです。** 画面の差分を読むだけで済ませると誰も気づきません。
必ず `gh pr checkout <PR番号>` して手元で動かし、実際に遊んでからコメントを書いてください。
