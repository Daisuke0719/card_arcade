...
中央に置かれた2枚の台札へ、数字が1つ違うカードを手札から次々に出していく速さを競います。
CPU と同時進行で出し合い、先に手札と山札を出し切ったほうが勝ちます。

## プレイ構成

- **人数**: プレイヤー1人 + CPU1人（`minPlayers: 2` / `maxPlayers: 2`）
- **使うカード**: `createDeck()` の52枚。ジョーカーは使いません
- **配り方**: 52枚をシャッフルして26枚ずつに分けます。各自の26枚の内訳は次のとおりです
  - 上から4枚: **手札**（常に4枚を保つ）
  - 次の1枚: **台札**として中央に表向きで置く（2人分で台札は2枚）
  - 残り21枚: **山札**（裏向き。手札と台札の補充に使う）
...
以下を今回実装するルールとし、記載のないローカルルールは扱いません。

1. 手札のカードは、**どちらかの台札と数字が1つ違い**のときだけ出せます。
2. **同じ数字は出せません**（台札が `7` のとき、手札の `7` は出せない）。
3. **A と K はつながります。** K の台札には A を、A の台札には K を出せます。判定には `@core` の `cycleRank` を使います。
...
9. **両者とも出せるカードが1枚もないとき（デッドロック）**は、`REFILL_DELAY_MS`（700ms）後に、
   **各自の山札から1枚ずつ台札に置き**、新しい台札にします（左はプレイヤーの山札、右は CPU の山札から出します）。
...
| 人間同士の対戦、オンライン対戦                 | CPU版では実装しない。**プレイヤー1人 + CPU1人のみ**。オンライン版は #41 に従う |
| ドラッグ&ドロップで台札を選んで出す            | 実装しない。**クリック1回・両方出せるときは左固定**                       |
| 同時にカードを出した場合、速いほうを有効にする | 実装しない。プレイヤーの入力と CPU のタイマーは、それぞれ独立して処理する |
...
| デッドロックを申告するボタン                   | 実装しない。デッドロックは自動で検出する                                  |
...
`logic.ts` に置く型と関数のシグネチャを示します。関数の実装は含まれていません。
...
`src/core/index.ts` と `src/components/index.ts` で公開されているものを使います。

### @core

| 名前                                            | 用途                                                          |
| ----------------------------------------------- | ------------------------------------------------------------- |
| `createDeck`                                    | ジョーカーを除く52枚のデッキを作る                            |
| `shuffle` / `createRng`                         | seed を固定し、同じ配り方を再現する                           |
| `cycleRank`                                     | 数字が1つ違うかを判定する。A と K のつながりもこの関数で扱う  |
| `draw` / `drawMany`                             | 山札から手札へ補充する                                        |
| `rankByScore`                                   | 残り枚数で順位を作る（`"lower-is-better"` を渡す）            |
| `useCpuTurn`                                    | 画面側で、待ち時間を設けて自動処理を実行する（記述は1行のみ） |
...
| `Button`           | 「もう一度」などの補助操作                         |
| `GameInstructions` | 遊び方の短い説明を画面に表示する（任意）           |
...
| `K の台札には A を出せる`                        | `cycleRank` を使った K と A のつながり            |
| `A の台札には K を出せる`                        | A から K への向きでも出せること                   |
