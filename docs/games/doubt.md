...
この文書に記載したルールを実装します。Issue と内容が異なる場合は、講師に確認してください。
...
| 項目       | 内容                                                        |
| ---------- | ----------------------------------------------------------- |
| 人数       | 4人（プレイヤー1人 + CPU 3人）。`createSoloVsCpu(3)` で作る |
| 使うカード | 52枚（ジョーカーなし）。`createDeck()`                      |
| 配り方     | 4人に13枚ずつ配り切る。`deal(deck, 4, 13)`                  |
| 先手       | プレイヤー（`you`）から始める                               |
| 手番の順序 | `you` → `cpu-1` → `cpu-2` → `cpu-3` の順で繰り返す          |
...
- 宣言ランクは自動で決まります。ゲーム開始時は `A` です。誰もダウトしなければ、次の人の宣言は1つ上がります
  （`A` → `2` → … → `K` → `A`）。`cycleRank(rank, 1)` で循環し、プレイヤーは宣言ランクを選べません。
- **手番の人は、手札から1〜4枚を選んで裏向きに場へ出します。** 出すカードは何でもかまいません
  （宣言ランクと違うカードを混ぜてもよい ＝ うそをつける）。
- 手番のプレイヤーは必ずカードを出します。パスはできません。手札が1枚の場合は1枚出します。
- **出した直後、他のプレイヤーが手番順に1人ずつ「ダウトするか」を決めます。**
  出した人の左隣から確認し、最初にダウトした1人で確定します。それ以降の人には確認しません。
  すでに上がった人には聞きません。
- ダウトが起きたら、直前に出された組だけを表向きに公開します。
  - 公開したカードが1枚でも宣言ランクと違えば「ダウト当たり」となり、出した人が場札をすべて引き取る
  - すべて宣言ランクと一致していれば「ダウト外れ」となり、ダウトした人が場札をすべて引き取る
- 引き取りの対象は常に場札のすべてです。
- 引き取りが起きたら場札を空にし、宣言ランクを `A` に戻します。
  次の手番は、引き取った人の次の人から始まります。
- **誰もダウトしなければ**、出されたカードは裏向きのまま場札に積まれ、宣言ランクが1つ進み、次の人の手番になります。
- 他のプレイヤーの手札は枚数だけを表示します。`Hand variant="hidden"` を使い、実カードをDOMに含めません。
...
- 残り1人になったらゲーム終了です。最後まで残った1人が最下位となり、上がった順に順位が付きます。
- CPU は、「宣言ランクのカードを何枚持っているか」と「場札の枚数」だけで判断します。
  複雑な推論や履歴の記憶は実装しません。
...
CPU の判断は `cpu.ts` に分けます（確率を返す純粋関数と、乱数を使って判定する処理）。
...
| `createSoloVsCpu`                                                                                 | プレイヤー + CPU 3人の一覧を作る                       |
| `createTurnState`                                                                                 | 手番の状態を作る（`you` を先手にする）                 |
| `nextTurn`                                                                                        | 次の人へ手番を移す（上がった人は対象外）               |
...
| `Hand`（`variant="open"`）   | プレイヤーの手札。`selectedIds` で複数選択する                    |
| `Hand`（`variant="hidden"`） | 他のプレイヤーの手札を枚数だけ表示する                            |
| `DeckPile`                   | 場札の山（裏向き・枚数表示）                                      |
| `Card`                       | `revealing` 中に公開されたカードを表示する                        |
| `Button`                     | 「出す」「ダウト！」「見送る」                                    |
...
`logic.test.ts` に、次の `it` の文字列を記載どおりに使用します。
雛形に含まれる3件も残します。

| `it` の文字列                                      | 確認する内容                                                      |
| -------------------------------------------------- | ----------------------------------------------------------------- |
| `宣言と実体が一致していればダウトは外れ`           | 宣言と実際のカードが一致するとき、`isBluff` が `false` を返すこと |
| `宣言と違うカードが混ざっていればダウトは当たり`   | 「1枚でも違えばうそ」という判定の境界                             |
| `ダウトが当たると出した人が場札を全部引き取る`     | ダウトが当たった場合の引き取り先と枚数                            |
| `ダウトが外れるとダウトした人が場札を全部引き取る` | ダウトが外れた場合の引き取り先と枚数                              |
| `解決後は宣言が A に戻る`                          | 引き取り後の再開条件。場札が0枚になることも同時に確認する         |
| `toPublicState に他人の手札が含まれない`           | 公開状態に他のプレイヤーの手札が含まれないこと                    |
