...
CIのSummary、Issue、講師から案内された番号を索引から探してください。

## 使い方

1. まず `npm run doctor` を実行する（環境に関する主な問題を確認できます）
2. 症状に近いものを下の索引から探す
3. 「対処」のコマンドを**上から順に**そのまま実行する
4. それでも解決しなければ、**作業を止めて講師に相談する**（別の方法で回避しないでください）

コマンドはすべて PowerShell にそのまま貼り付けて動きます。

**このページのコマンドは、特に指定がない限り「ターミナルB」（参加者が操作する窓）で実行します。**
...
| ハーネス | T-35 | 他の人のブランチにいるときに、そのコードを直そうとして止められた |
...
| CI | T-21 | 手元では成功するのにCIの `verify` が失敗する |
...
**`npm install` は使わないでください。** `package-lock.json` が変わるとCIが失敗します（T-22）。
...
ターミナルBで5173番ポートを使用していると、別の開発サーバーが5174番ポートで起動することがあります。
...
短く、ASCII文字だけで構成されたパスへcloneし直してください。
...
**別の方法で制限を回避しようとしないでください（`sed -i`・リダイレクト・`cp` での上書きなど）。**
...
2. 環境変数 `CARD_ARCADE_HARNESS=off` が残っている（**講師が緊急時に使う設定**です。参加者は使いません）
...
範囲違反や純粋性違反は、遅くともPull RequestのCIで検出されます。
...
**`git add .` は使わないでください。** 担当外のファイルが含まれ、範囲チェックが失敗する原因になります。

**`--no-verify` は使えません。** ターミナルA では `guard-bash.mjs` が止めますし、
手元の確認を省略しても、CIでは同じスクリプトが実行されます。表示された手順に沿って修正してください。
...
ターミナルAには、開発サーバーの起動ではなく、確認手順の作成を依頼します。
...
**別の方法で回避しないでください。** B の窓に切り替えて、決められた手順を実行してください。
...
### T-35. 他の人のブランチにいるときに、そのコードを直そうとして止められた

**症状**
`gh pr checkout <他の人のPR番号>` などで他の人のブランチに移った状態で修正を頼むと、こう返ってくる。
...
`guard-scope.mjs` はブランチ名だけでなく**この記録**とも突き合わせるので、
ブランチが他の人のものになっている間も、担当を見失いません。

これが無いと、他の人のブランチで「ついでに直しておきました」ができてしまいます。
そうなると**作った本人が自分のコードを説明できなくなります。**
...
**`owner.json` が無い、または中身が違う場合**（雛形を作る前にブランチを移った、など）は、
...
`id` `name` `owner` `difficulty` は**運営が決めた値**です。変更すると契約テストとCIが失敗します。

**対処**

まず、画面上部の赤いエラー欄を確認します。原因と対象のフォルダ名が表示されています。
...
これは警告（warn）であり、CIは失敗しません。担当フォルダ内に限られる問題なので、
機械では止めず、**警告として画面に出すだけ**にしてあります。
...
`ready` に変更するのは、実際の画面で最初から最後まで1回プレイできた後です。
...
| **依存の変更チェックが別にある** | `package.json` / `package-lock.json` の変更は専用ステップで検出されます（→ T-22） |

まず、修正後のコミットをpushしているか確認してください。
...
これを実行してもCIが成功しない場合は、ほかの変更が含まれていないか確認し、講師に相談してください。
...
**待っている間に手を止めないでください。** `README.md` の実装メモを書くなど、
先に進められる作業があります。
...
古いPull Requestは、参加者がGitHubの画面から閉じてください。
...
これらは対話形式のため、参加者がターミナルBで実行してください。
...
**原因**
まず、collaboratorの招待を承諾しているか確認してください。
...
**マージするのは講師です。** 上の2つが揃っていれば、あとは講師がマージします。
...
Ready にしても押せない場合は、講師へ声をかけてください。

講師がマージするときは**必ず Squash** を選びます。
...
`npm run scope` が成功することを確認してください。
...
このページの手順を試しても解決しない場合は、**別の方法で回避しようとせず、講師に相談してください。**
この手順で解決しない場合は、別の回避策を試さず講師に相談してください。

1. ターミナルAで、現在の状態を確認して整理する
   （`npm run scope` / `npm test` の結果を集め、コードは変更しません）
2. その出力をそのまま講師に見せる
3. Issueに `blocked` ラベルを付け、作業が止まっていることを共有する

作業が止まった場合は、状況を整理して講師に相談してください。
