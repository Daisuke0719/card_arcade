 ...
 *
 * 「最初にやること」に載せるプロンプトは docs/handson-steps.md の Step 5〜7 と同じ文面です。
...
\`npm run scaffold -- --game ${item.gameId}\` は、雛形5ファイルの生成と同時に
...
> 自分の雛形を作るときは、**必ず \`--game ${item.gameId}\` でゲームIDを1つだけ**指定します。
...
> 3. **\`Closes ${issueRef}\` になっているか**（番号が違うと、他人の Issue を閉じてしまいます）
>
